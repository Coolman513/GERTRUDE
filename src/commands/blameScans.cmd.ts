...
export const BlameScansCmd = async (client: Client, db: Database, dbdata: DatabaseData, interaction: ChatInputCommandInteraction) => {
  ...
  if (projects[project].scans[pos].abbreviation == null)
    return fail(`Scans have not been added.`, interaction);
  
  let status = '';
  let success = false;

  let projObj = projects[project];
    if ((pnumber != null && projObj.pnumber === pnumber) || (pnumber == null && projObj.scansDone == false)) {
      success = true;
