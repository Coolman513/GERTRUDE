...
export const AddpnumCmd = async (client: Client, db: Database, dbdata: DatabaseData, interaction: ChatInputCommandInteraction) => {
  ...
  let epref = ref.child('pnumber').push({
    number: number,
    done: false,
    additionalStaff: [],
    tasks: []
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`I added pnum ${number} to \`${project}\` for you.`)
    .setColor(0xd797ff);
